...
export async function uploadPhotoToDrive(dataUrl, filename) {
  ...
  return {
    ...
    // Dipakai untuk tombol "Download PDF" / unduh file apa adanya.
    downloadUrl: created.data.webContentLink || `https://drive.google.com/uc?id=${created.data.id}&export=download`,
    // Format resmi Google Drive untuk menampilkan PDF (atau dokumen lain)
    // langsung di halaman web lewat <iframe>, tanpa perlu diunduh dulu.
    previewUrl: `https://drive.google.com/file/d/${created.data.id}/preview`,
  };
}
