...
const ACTIVITY_CATEGORIES = [
  {
    ...
    methods: [
      {
        key: "non_tatap_muka",
        label: "Non Tatap Muka",
        sublabel: "Tidak mendapat poin — pencatatan saja",
        earnsPoints: false,
        types: [
          { key: "whatsapp", label: "WhatsApp", points: 0 },
          { key: "sosmed", label: "Sosmed", points: 0 },
          { key: "call", label: "Call", points: 0 },
        ],
      },
      {
        key: "tatap_muka",
        label: "Tatap Muka",
        sublabel: "Zoom / Offline — mendapat poin",
        earnsPoints: true,
        types: [
          ...
          { key: "presentation", label: "Presentation", points: 5 },
          {
            ...
            noteLabel: "Nomor Polis",
            noteInputType: "text", // isian satu baris, bukan textarea paragraf
          },
        ],
      },
    ],
...
/**
 * Label untuk jenis aktivitas LAMA yang sudah tidak ada di konfigurasi di
 * atas — supaya data historis tetap terbaca (tidak tampil sebagai kode
 * mentah) di Riwayat, Ringkasan, dan Laporan Analitik.
 */
const LEGACY_TYPE_LABELS = {
  whatsapp_call: "WhatsApp Chat / Call",
};
...
export async function saveMembers(members) {
  ...
  if (!ok) return ok;
  await pushMembersToSheets(members);
  return ok;
}
...
export async function deleteMember(id, requesterEmail) {
  const members = getMembers();
...
/** Daftar metode (Non Tatap Muka / Tatap Muka) untuk satu jalur. */
export function getActivityMethods(categoryKey) {
  const cat = ACTIVITY_CATEGORIES.find((c) => c.key === categoryKey);
  return cat ? cat.methods : [];
}

/**
 * Mencari konfigurasi (label, poin, dst) untuk satu jenis aktivitas —
 * dicari lintas metode, jadi pemanggil tidak perlu tahu jenis ini ada di
 * metode Non Tatap Muka atau Tatap Muka.
 *
 ...
 *
 * Kalau jenisnya sudah tidak ada di konfigurasi (data lama), tetap
 * dikembalikan label historisnya supaya riwayat tidak tampil sebagai
 * kode mentah.
 */
export function getActivityTypeConfig(categoryKey, typeKey) {
  ...
  if (cat) {
    for (const method of cat.methods) {
      const type = method.types.find((t) => t.key === typeKey);
      if (type) {
        return {
          ...type,
          methodKey: method.key,
          methodLabel: method.label,
          earnsPoints: method.earnsPoints,
  ...
  if (LEGACY_TYPE_LABELS[typeKey]) {
    return {
      key: typeKey,
      label: LEGACY_TYPE_LABELS[typeKey],
      points: 0,
      legacy: true,
...
/** Admin menandai aktivitas sebagai Tidak Valid (mis. data tidak sesuai/mencurigakan). */
export function markActivityInvalid(id, adminName) {
  return updateActivity(id, {
    validated: false,
    validatedAt: null,
    validatedBy: null,
    ...
    invalidBy: adminName || "Admin",
  });
}

/** Admin mengembalikan status ke "Menunggu Validasi" (batal Valid atau batal Tidak Valid). */
...
export function getMemberPointsSummary(memberId) {
  ...
  activities.forEach((a) => {
    ...
    else if (a.invalid) invalidPoints += pts;
    else unconfirmedPoints += pts;
    const key = `${a.category}:${a.type}`;
    counts[key] = (counts[key] || 0) + 1;
  });
  return { validPoints, unconfirmedPoints, invalidPoints, counts };
}
...
export function summarizePoints(activities) {
  ...
  return { validPoints, unconfirmedPoints, invalidPoints };
}
...
/** Mengecek apakah password polos cocok dengan password tersimpan milik satu member. */
export function verifyMemberPassword(memberId, plainPassword) {
  const member = getMembers().find((m) => m.id === memberId);
  if (!member) return false;
  return isHashed(member.password)
    ? verifyPassword(plainPassword, member.password)
    : plainPassword === member.password; // fallback untuk data lama yang belum ter-hash
}

/**
 * Dipakai halaman "Kelola Profil" — member mengganti password mereka
 * sendiri. Wajib memasukkan password lama yang benar dulu.
 */
export async function changeOwnPassword(memberId, currentPassword, newPassword) {
  if (!verifyMemberPassword(memberId, currentPassword)) {
    throw new Error("Password saat ini salah.");
  }
  await updateMember(memberId, { password: newPassword });
  return true;
}
...
function sheetRowToActivity(row) {
  ...
  return {
    ...
    invalid: row.invalid === true || row.invalid === "TRUE" || row.invalid === "true",
  };
}
