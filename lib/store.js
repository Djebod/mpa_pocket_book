...
/**
 * Apakah member ini masih terdaftar (belum dihapus Admin)? Dipakai untuk
 * memvalidasi ulang sesi login (lihat app/providers.js) maupun sebagai
 * guard terakhir sebelum menyimpan aktivitas baru (lihat addActivity di
 * bawah), supaya member yang sudah dihapus tidak bisa lagi melakukan
 * pencatatan aktivitas walau sesi lokalnya di browser belum ter-clear.
 */
export function isMemberActive(memberId) {
  if (!memberId) return false;
  return getMembers().some((m) => m.id === memberId);
}
...
export function addActivity(activity) {
  if (!isMemberActive(activity?.memberId)) {
    throw new Error(
      "Akun ini sudah tidak terdaftar (kemungkinan dihapus oleh Admin), sehingga tidak bisa mencatat aktivitas baru. Silakan logout dan hubungi Admin kalau ini keliru."
    );
  }
