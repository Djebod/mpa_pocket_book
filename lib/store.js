...
function seedIfEmpty() {
  ...
  if (window.localStorage.getItem(KEYS.warisan) === null) {
    writeJSON(KEYS.warisan, null);
  }
...
// ---------- Konsep Warisan (halaman tunggal) ----------

export function getWarisanInfo() {
  seedIfEmpty();
  return readJSON(KEYS.warisan, null);
}

export function saveWarisanInfo(data) {
  const ok = writeJSON(KEYS.warisan, data);
  pushWarisanToSheets(data);
  if (!ok) throw new Error("Data Konsep Warisan gagal disimpan: penyimpanan browser penuh.");
  return data;
}
...
function warisanToSheetRow(c) {
  if (!c) return { id: "main", description: "", files: "[]" };
  return { ...c, id: "main", files: filesFieldToSheetValue(c.files) };
}

function sheetRowToWarisan(row) {
  if (!row) return null;
  return { ...row, files: sheetValueToFilesField(row.files) };
}
...
function pushWarisanToSheets(data) {
  pushToSheets("/api/warisan", { warisan: warisanToSheetRow(data) }, "Warisan");
}
...
export async function syncAllFromSheets() {
  ...
  try {
    const [mRes, pRes, aRes, promoRes, tutRes, comRes, warRes, medRes, recRes] = await Promise.all([
      ...
      fetch("/api/warisan").then((r) => r.json()),
    ...
    if (warRes.configured) {
      configured = true;
      writeJSON(KEYS.warisan, warRes.warisan ? sheetRowToWarisan(warRes.warisan) : null);
    }
