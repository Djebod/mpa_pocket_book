...
function seedIfEmpty() {
  ...
  if (window.localStorage.getItem(KEYS.afterSalesClaim) === null) {
    writeJSON(KEYS.afterSalesClaim, null);
  }
}
...
// ---------- After Sales & Claim (halaman tunggal, multi-file) ----------

export function getAfterSalesClaimInfo() {
  seedIfEmpty();
  return readJSON(KEYS.afterSalesClaim, null);
}

export function saveAfterSalesClaimInfo(data) {
  const ok = writeJSON(KEYS.afterSalesClaim, data);
  pushAfterSalesClaimToSheets(data);
...
function afterSalesClaimToSheetRow(c) {
  if (!c) return { id: "main", files: "[]" };
  return { ...c, id: "main", files: filesFieldToSheetValue(c.files) };
}

function sheetRowToAfterSalesClaim(row) {
  if (!row) return null;
  return { ...row, files: sheetValueToFilesField(row.files) };
}
...
function pushAfterSalesClaimToSheets(data) {
  pushToSheets(
    "/api/after-sales-claim",
    { afterSalesClaim: afterSalesClaimToSheetRow(data) },
    "AfterSalesClaim"
  );
}
...
export async function syncAllFromSheets() {
  ...
  try {
    const [mRes, pRes, aRes, promoRes, tutRes, recRes, akRes, kkRes, ascRes] = await Promise.all([
    ...
    if (ascRes.configured) {
      configured = true;
      writeJSON(
        KEYS.afterSalesClaim,
        ascRes.afterSalesClaim ? sheetRowToAfterSalesClaim(ascRes.afterSalesClaim) : null
      );
    }
