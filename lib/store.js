...
// Konfigurasi aktivitas baru: 2 kategori, tiap kategori punya 3 jenis
// aktivitas dengan bukti & poin masing-masing. Poin baru "aktif" (masuk
// hitungan Valid Point) setelah divalidasi Admin lewat tombol "Valid".
const ACTIVITY_CATEGORIES = [
  {
    key: "nasabah",
    label: "Calon Nasabah",
    sublabel: "Jalur Penjualan",
    types: [
      { key: "fact_finding", label: "Fact Finding", points: 3, proofType: "photo", proofLabel: "Foto bersama calon nasabah" },
      { key: "presentation", label: "Presentation", points: 5, proofType: "photo", proofLabel: "Foto bersama calon nasabah dengan proposal terlihat" },
      { key: "closing", label: "Closing", points: 10, proofType: "policy", proofLabel: "Nomor polis (+ foto opsional)" },
    ],
  },
  {
    key: "agen",
    label: "Calon Agen",
    sublabel: "Jalur Rekrutmen",
    types: [
      { key: "fact_finding", label: "Fact Finding", points: 3, proofType: "photo", proofLabel: "Foto bersama calon agen" },
      { key: "presentation", label: "Presentation", points: 5, proofType: "photo", proofLabel: "Foto bersama calon agen dengan materi presentasi terlihat" },
      { key: "recruit", label: "Recruit", points: 10, proofType: "photo", proofLabel: "Foto bukti transfer pembayaran ujian AAJI" },
    ],
  },
];
...
const SEED_PRODUCTS = [
  {
    ...
    description:
      ...
      "Ilustrasi: Nasabah usia 30 tahun, premi bulanan mulai Rp 350.000, plafon rawat inap harian Rp 500.000, santunan rawat jalan Rp 100.000 per kunjungan (maks. 20 kunjungan/tahun).\n\n" +
      "Cara menjual: gali kebutuhan calon nasabah (apakah sudah punya BPJS/asuransi kantor?), tunjukkan gap perlindungan, jelaskan manfaat dengan ilustrasi angka nyata, tawarkan simulasi premi, tutup dengan ajakan mengisi formulir hari itu juga.",
    files: [],
  },
  {
    id: "prod_2",
    ...
    description:
      ...
      "Ilustrasi: Uang pertanggungan Rp 500.000.000, usia masuk 35 tahun, masa asuransi 15 tahun, premi tahunan mulai Rp 4.200.000. Tersedia rider kecelakaan dan pembebasan premi.\n\n" +
      "Cara menjual: mulai dengan pertanyaan reflektif soal kondisi keuangan keluarga jika terjadi risiko, hitung kebutuhan uang pertanggungan bersama nasabah, bandingkan dengan tabungan biasa, sampaikan proses klaim yang sederhana, ajukan penutupan.",
    files: [],
  },
  {
    id: "prod_3",
    ...
    description:
      ...
      "Ilustrasi: Anak usia 3 tahun, target dana kuliah Rp 200.000.000 di usia 18 tahun, premi bulanan mulai Rp 1.100.000 (tergantung usia & masa bayar orang tua).\n\n" +
      "Cara menjual: tanyakan usia anak dan rencana pendidikan orang tua, tunjukkan proyeksi kenaikan biaya pendidikan, jelaskan skema pencairan bertahap, tekankan manfaat bebas premi, tutup dengan simulasi dana.",
    files: [],
  },
  {
    id: "prod_4",
    ...
    description:
      ...
      "Ilustrasi: Usia masuk 30 tahun, target pensiun usia 55 tahun, iuran bulanan mulai Rp 500.000, estimasi dana terkumpul dan anuitas bulanan dapat dilihat pada tabel ilustrasi resmi produk.\n\n" +
      "Cara menjual: ajak calon nasabah membayangkan kebutuhan bulanan di masa pensiun, jelaskan konsep bunga majemuk, tunjukkan simulasi dana terkumpul, sampaikan fleksibilitas penarikan, tutup dengan rekomendasi iuran.",
    files: [],
  },
];
...
function seedIfEmpty() {
  ...
  if (window.localStorage.getItem(KEYS.medical) === null) {
    writeJSON(KEYS.medical, null);
  }
...
export function getActivityTypeConfig(categoryKey, typeKey) {
  ...
  if (!cat) return null;
  return cat.types.find((t) => t.key === typeKey) || null;
}
...
// ---------- Tabel Medical (halaman tunggal) ----------

export function getMedicalInfo() {
  seedIfEmpty();
  return readJSON(KEYS.medical, null);
}

export function saveMedicalInfo(data) {
  const ok = writeJSON(KEYS.medical, data);
  pushMedicalToSheets(data);
  if (!ok) throw new Error("Data Tabel Medical gagal disimpan: penyimpanan browser penuh.");
  return data;
}
...
// Field lampiran (foto/PDF) pada produk sekarang berupa satu array
// `files` (sama seperti Promo) — di Sheets disimpan sebagai teks JSON di
// kolom `files`, dengan guard ukuran (lihat filesFieldToSheetValue di
// bawah) supaya file yang belum ter-upload ke Drive (masih base64
// raksasa) tidak memutus sinkronisasi.
...
// Dipakai untuk field `files` (array lampiran) pada Promo, Medical, dan
...
function pushMedicalToSheets(data) {
  pushToSheets("/api/medical", { medical: medicalToSheetRow(data) }, "Medical");
}

function pushRecruitToSheets(data) {
  pushToSheets("/api/recruit", { recruit: recruitToSheetRow(data) }, "Recruit");
}
...
export async function syncAllFromSheets() {
  ...
  try {
    const [mRes, pRes, aRes, promoRes, tutRes, medRes, recRes] = await Promise.all([
      fetch("/api/members").then((r) => r.json()),
      fetch("/api/products").then((r) => r.json()),
      fetch("/api/activities").then((r) => r.json()),
      fetch("/api/promo").then((r) => r.json()),
      fetch("/api/tutorials").then((r) => r.json()),
      fetch("/api/medical").then((r) => r.json()),
    ...
    if (medRes.configured) {
      configured = true;
      writeJSON(KEYS.medical, medRes.medical ? sheetRowToMedical(medRes.medical) : null);
    }
