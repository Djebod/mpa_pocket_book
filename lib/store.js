...
export function saveKomisiKompensasiInfo(data) {
  const ok = writeJSON(KEYS.komisiKompensasi, data);
  pushKomisiKompensasiToSheets(data);
...
// Produk sekarang punya 4 field lampiran satu-file terpisah (Materi
// Training Versi Manulife Pusat, Materi Training Versi MPA, Tabel
// Premi, Tabel Medical) — masing-masing disimpan sebagai teks JSON di
// kolomnya sendiri di Sheets, dengan guard ukuran yang sama seperti
// field lampiran lain (kalau belum ter-upload ke Drive, masih base64
// raksasa, jangan dikirim supaya tidak memutus sinkronisasi). Field
// `resume` beda sendiri — bisa lebih dari satu file, jadi disimpan
// sebagai array (sama seperti Promo/Recruit).
const PRODUCT_FILE_FIELDS = ["materiTrainingManulife", "materiTrainingMPA", "tabelPremi", "tabelMedical"];
...
function productToSheetRow(p) {
  ...
  row.resume = filesFieldToSheetValue(row.resume);
  return row;
}
...
function sheetRowToProduct(row) {
  ...
  product.resume = sheetValueToFilesField(row.resume);
  return product;
}
...
export async function syncAllFromSheets() {
  ...
  try {
    const [mRes, pRes, aRes, promoRes, tutRes, recRes, akRes, kkRes] = await Promise.all([
