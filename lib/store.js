...
export function deleteAfterSalesClaimEntry(id) {
  const next = getAfterSalesClaimList().filter((e) => e.id !== id);
  saveAfterSalesClaimList(next);
  return next;
}
