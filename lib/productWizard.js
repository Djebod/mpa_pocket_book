...
/**
 * Mencari produk di Katalog Produk (data admin, `store.getProducts()`)
 * yang judulnya paling mirip dengan entri KB wizard (berdasarkan kode
 * produk atau kemiripan kata pada nama). Dipakai supaya kartu hasil di
 * halaman Rekomendasi Produk bisa link langsung ke Detail Produk kalau
 * memang sudah ada di katalog — kalau tidak ketemu, return null (tombol
 * jadi tidak bisa ditekan).
 */
export function matchCatalogProduct(products, code, name) {
  if (!products || products.length === 0) return null;
  const codeKey = code.toLowerCase().replace(/[^a-z0-9]/g, "");
  ...
  for (const p of products) {
    ...
    const productTokens = normalizeTokens(p.name);
    let score = 0;
    if (codeKey && productKey.includes(codeKey)) score += 5;
    score += nameTokens.filter((t) => productTokens.includes(t)).length;
    if (score > bestScore) {
      bestScore = score;
      best = p;
    }
