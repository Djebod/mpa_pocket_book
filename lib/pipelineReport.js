...
const NEXT_ACTION_BY_TYPE = {
  whatsapp:
    "Baru kontak lewat WhatsApp — tindak lanjuti dengan mengajak bertemu (Zoom/offline) untuk mulai menggali kebutuhan (Fact Finding).",
  sosmed:
    "Baru kontak lewat sosial media — bangun percakapan lebih personal, lalu ajak bertemu (Zoom/offline) untuk Fact Finding.",
  call:
    "Baru kontak lewat telepon — tindak lanjuti dengan mengatur jadwal pertemuan (Zoom/offline) untuk menggali kebutuhan.",
  whatsapp_call:
    "Baru kontak jarak jauh — tindak lanjuti dengan mengajak bertemu (Zoom/offline) untuk mulai menggali kebutuhan (Fact Finding).",
  fact_finding:
...
export function buildPipelineReport(activities, contacts, opts = {}) {
  ...
  const NON_TATAP_MUKA = ["whatsapp", "sosmed", "call", "whatsapp_call"];
  const baruNonTatapMuka = rows.filter((r) => NON_TATAP_MUKA.includes(r.lastTypeKey));
  if (baruNonTatapMuka.length > 0) {
    highlights.push({
      level: "info",
      title: `${baruNonTatapMuka.length} prospek masih sebatas kontak non tatap muka`,
      detail:
        "Belum ada pertemuan (Zoom/offline), jadi belum menghasilkan poin. Ajak bertemu untuk mulai menggali kebutuhan — kontak jarak jauh saja jarang berujung closing.",
      names: baruNonTatapMuka.slice(0, 5).map((r) => r.name),
    });
  }
