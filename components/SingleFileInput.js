...
/**
 * Input lampiran satu file (foto ATAU PDF) untuk satu field spesifik
 * produk (Materi Training, Tabel Premi, Resume, Tabel Medical). `value`
 * berbentuk { url, downloadUrl, previewUrl, mimeType, name } atau null.
 */
export default function SingleFileInput({ value, onChange, label }) {
  ...
  return (
    <div>
      ...
      {value ? (
        <div className="flex items-center gap-3 mb-2">
          {isImageValue && (
            ...
            <img src={value.url} alt={value.name || ""} className="w-14 h-14 rounded object-cover border border-ink/10" />
          )}
          {isPdfValue && (
            <div className="w-14 h-14 rounded border border-ink/10 bg-paper-dark/40 flex items-center justify-center text-xl">
              📄
            </div>
          )}
          <div className="text-xs text-ink/60 min-w-0">
            <p className="truncate max-w-[220px]">{value.name || "File terlampir"}</p>
            {driveHosted ? (
      ...
      >
        {processing ? "Memproses…" : value ? "Ganti File" : "Unggah PDF / Foto"}
      </button>
