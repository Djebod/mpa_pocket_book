...
export async function GET() {
  if (!isSheetsConfigured()) {
    return NextResponse.json({ configured: false, medical: null });
  }
  try {
    const rows = await readSheet("Medical");
    return NextResponse.json({ configured: true, medical: rows[0] || null });
  } catch (err) {
...
export async function POST(request) {
  ...
  try {
    const { medical } = await request.json();
    await writeSheet("Medical", medical ? [medical] : []);
