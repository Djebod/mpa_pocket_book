...
export async function GET() {
  if (!isSheetsConfigured()) {
    return NextResponse.json({ configured: false, commission: null });
  }
  try {
    const rows = await readSheet("Commission");
    return NextResponse.json({ configured: true, commission: rows[0] || null });
  } catch (err) {
...
export async function POST(request) {
  ...
  try {
    const { commission } = await request.json();
    await writeSheet("Commission", commission ? [commission] : []);
