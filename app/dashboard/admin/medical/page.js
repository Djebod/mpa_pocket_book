...
export default function AdminMedicalPage() {
  ...
  useEffect(() => {
    const existing = store.getMedicalInfo();
    if (existing) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    store.saveMedicalInfo({ description, files });
    setSaved(true);
  ...
  return (
    <div>
      <h1 className="font-display italic text-2xl sm:text-3xl text-ink mb-1">Kelola Tabel Medical</h1>
      <p className="text-sm text-ink/60 mb-8">
        Halaman ini akan ditampilkan ke seluruh member — isi deskripsi tabel medical dan lampirkan file
      ...
      <form onSubmit={handleSubmit} className="bg-card border border-ink/10 rounded-lg shadow-stamp px-4 sm:px-6 py-5 sm:py-6 perforated relative">
        ...
        <div className="mb-5">
          <label className="block text-sm font-semibold text-ink mb-1.5">Tabel Medical</label>
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            rows={5}
            placeholder="Contoh: Ini adalah halaman Tabel Medical Mulia Putri Agency..."
